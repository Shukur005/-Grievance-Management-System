...
if (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity'] > $timeout)) {
    session_unset();
    session_destroy();
    header("Location: userlogin1.html?timeout=1");
    exit();
}
...
try {
    ...
    // Get user details
    $stmt = $conn->prepare("SELECT email, phone, gender, pass FROM userregistration WHERE username = ?");
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as pend FROM complaints WHERE user = ? AND pending = '1'");
    $stmt->bind_param("s", $user);
    $stmt->execute();
    $result = $stmt->get_result();
    $pendingCount = $result->fetch_assoc();
    $pend = $pendingCount['pend'];

    // Get completed complaints count
    $stmt = $conn->prepare("SELECT COUNT(*) as compl FROM completedcomp WHERE user = ?");
    $stmt->bind_param("s", $user);
    $stmt->execute();
    $result = $stmt->get_result();
    $completedCount = $result->fetch_assoc();
    $compl = $completedCount['compl'];

    // Get in-progress complaints count
    $stmt = $conn->prepare("SELECT COUNT(*) as inpro FROM inprogresscomp WHERE user = ?");
    $stmt->bind_param("s", $user);
    $stmt->execute();
    $result = $stmt->get_result();
    $inProgressCount = $result->fetch_assoc();
    $inpro = $inProgressCount['inpro'];
